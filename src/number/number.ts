 ...
 * @return {boolean}
 */
export function inRange(inNum: number, range: [number, number], openInterval = false) {
 ...
 * @return {number}
 */
export function normaliz(inNum: number, inRange: [number, number], outRange: [number, number] = [0, 1]) {
    let newNum: number
    let inMin = inRange[0]
    let inMax = inRange[1]
