...
describe("treeEach()", () => {
    ...
    test("遍历循环引用对象", () => {
        ...
        // 循环引用时的回调
        function checkCycleCallback(value, key, parent, path, firstPath) {
            countCycle++;
        }
    ...
    test("depthReboundFunc", () => {
        ...
        rc.object.objectEach(obj, (value, key, info) => {
            ...
            depthFirst: true,
            needKeyPath: true,
            depthReboundFunc(value, key, info) {
        ...
        expect(count).toBe(10);
        expect(obj.children.itemA.length).toBe(4);
        expect(obj.children.itemA.children.itemA_1.length).toBe(1);
    });
    test("提前终止 广度搜索", () => {
        let count = 0;
        let obj = {
