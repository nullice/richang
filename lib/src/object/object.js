...
export function getObjectHash(object) {
    return _objectHash(object, { respectType: false });
}
/**
 * 对象是否为空
 * @returns {boolean}
 ...
 * @return {string[]}
 */
export function normalizeKeyPath(inPath) {
 ...
 * @return {boolean}
 */
export function keyPathEqual(keyPathA, keyPathB) {
 ...
 * @return {any}
 */
export function setObjectValueByPath(object, path, value, overwrite = false) {
    let keyPath = normalizeKeyPath(path);
    let cursor = object;
 ...
 * @param object
 */
export let cloneDeep = _cloneDeep;
/**
 * 对象映射。
 ...
 * @return {any}
 */
export function mappingObject(objectSource, mappingRule, reverse) {
    let newObject = {};
    objectEach(mappingRule, (value, key, info, CONTOL) => {
        ...
        if (info.end || valueIsArray) {
            ...
            if (valueIsArray) {
                ...
                if (reverse) {
                    let rawValue = getObjectValueByPath(objectSource, info.keyPath);
                    // 处理函数
                    if (reverseFunc) {
                    ...
                    setObjectValueByPath(newObject, rawKeyPath, rawValue, true);
                }
                else {
                    ...
                    setObjectValueByPath(newObject, info.keyPath, rawValue, true);
                }
                return CONTOL.continue;
            }
            // 直接映射
                ...
                if (reverse) {
                    ...
                    setObjectValueByPath(newObject, info.keyPath, getObjectValueByPath(objectSource, value), true);
                }
            }
        }
    }, { needKeyPath: true });
